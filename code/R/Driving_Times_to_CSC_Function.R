...
#' @param closest_five_PSCs Data frame.
...
GetDrivingTimeCSC<-function(closest_five_PSCs,facilities,warn = TRUE){
  ...
  facilities_PSC<-closest_five_PSCs%>%dplyr::select(-c("time","distance","latitude_origen","longitude_origen"))
  ...
  meters<-GCD(facilities_by_type_long_lat_PSC,facilities_by_type_long_lat_CSC)
  # Using the new set of hospitals
  facilities_by_type_long_lat_CSC<-facilities_by_type_long_lat_CSC[which(meters<sort(meters)[5]),]
  facilities_CSC<-facilities_CSC[which(meters<sort(meters)[5]),]
  
  
  
  # Coordinates for openrouteservice
  coordinates<-rbind(facilities_by_type_long_lat_PSC,facilities_by_type_long_lat_CSC)
    
    
  # Sources and destinations for openrouteservice
  # Indices start on zero
  sources<- c(0:(nrow(facilities_by_type_long_lat_PSC)-1))
  destinations<- c(nrow(facilities_by_type_long_lat_PSC):(nrow(coordinates)-1))
  ...
  CSC<-data.frame(time=time[1,index],
                     ...
                     facilities_CSC[index,])
    
  
  names(CSC)<-paste0(names(CSC),"_CSC")
  names(facilities_PSC)<-paste0(names(facilities_PSC),"_PSC")
 
  final_output<-cbind(CSC,facilities_PSC)
